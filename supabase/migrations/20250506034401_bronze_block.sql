/*
  ...
  1. Changes
    ...
    - Fix storage policies to ensure proper access
    
  2. Security
    - Maintain existing access controls
    - Allow proper public access to profile pictures
    - Ensure only users can manage their own pictures
*/
...
-- Create a function to directly update profile picture URL
CREATE OR REPLACE FUNCTION update_profile_picture_url(user_id UUID, new_url TEXT)
RETURNS BOOLEAN AS $$
DECLARE
  success BOOLEAN;
BEGIN
...
GRANT EXECUTE ON FUNCTION update_profile_picture_url TO authenticated;